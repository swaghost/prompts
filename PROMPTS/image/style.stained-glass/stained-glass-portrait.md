# Stained Glass Portrait Style

## Overview

Transform portrait photographs into luminous handcrafted stained glass artworks with Art Nouveau elegance—preserving the subject's recognizable identity while replacing every element with jewel-like glass panels, ornamental frames, and Gothic rose-window backgrounds.

**Style:** Premium editorial stained-glass identity portrait  
**Recommended Canvas:** 4:5 portrait orientation  
**Aesthetic:** Luminous handcrafted glass with dark lead came, botanical Gothic backgrounds, Art Nouveau ornamental frames

---

## Core Prompt

```
Transform the supplied 4:5 portrait canvas into a premium editorial stained-glass identity portrait. Preserve the supplied person's recognizable face, natural eyes, hairstyle, hair color, skin tone, age and expression. Replace every part of the canvas—including any blurred background—with luminous handcrafted stained glass: face, hair, clothing, radiant halo, botanical Gothic rose-window background and Art Nouveau ornamental frame, elegant dark lead came, jewel-like cobalt, turquoise, amber, ruby, emerald and violet glass, warm transmitted light. Centered chest-up symmetrical composition filling the canvas edge to edge. Attractive anatomically correct face; no text, logo, watermark, extra person, cracks or scars across facial features. Preserve the input image's native 4:5 canvas shape exactly.
```

---

## Prompt Breakdown

### Identity Preservation
- **Preserve the supplied person's:**
  - Recognizable face
  - Natural eyes
  - Hairstyle
  - Hair color
  - Skin tone
  - Age
  - Expression

**Critical:** The subject must remain identifiable—this is an artistic transformation, not a complete replacement.

---

### Complete Stained Glass Transformation

**Replace every part of the canvas with luminous handcrafted stained glass:**

**Face:**
- Translucent glass panels
- Natural skin tone rendered in glass
- Facial features preserved
- Warm transmitted light

**Hair:**
- Glass interpretation of hairstyle
- Original hair color maintained
- Flowing glass strands or sections
- Lead came defining sections

**Clothing:**
- Garments rendered in colored glass
- Jewel-tone panels
- Ornamental details
- Lead came outlining

**Radiant Halo:**
- Glowing ring or aura behind subject
- Luminous effect
- Religious/sacred art inspiration
- Enhances focal point

**Background:**
- Botanical Gothic rose-window design
- Intricate floral/botanical patterns
- Circular or geometric Gothic motifs
- Fills entire background space

**Frame:**
- Art Nouveau ornamental frame
- Organic flowing lines
- Decorative elements
- Integrates with overall design

---

### Glass Material Specifications

**Dark Lead Came:**
- Elegant black leading
- Defines glass panel boundaries
- Creates structure and definition
- Traditional stained glass aesthetic

**Jewel-Like Glass Colors:**
- **Cobalt:** Deep rich blue
- **Turquoise:** Bright blue-green
- **Amber:** Warm golden yellow-orange
- **Ruby:** Deep red
- **Emerald:** Rich green
- **Violet:** Purple tones

**Lighting:**
- Warm transmitted light
- Glowing from behind glass
- Creates luminous effect
- Mimics backlit church windows

---

### Composition Requirements

**Layout:**
- Centered chest-up symmetrical composition
- Fills canvas edge to edge
- No empty borders or margins
- Complete coverage

**Framing:**
- Chest-up (shoulders and head visible)
- Centered positioning
- Symmetrical balance
- Professional portrait framing

**Canvas Utilization:**
- Edge-to-edge coverage
- No wasted space
- Frame elements reach canvas borders
- Complete and finished look

---

### Quality Standards

**Anatomical Accuracy:**
- Attractive face
- Anatomically correct features
- Proper proportions
- Natural positioning

**Prohibitions:**
- **No text**
- **No logo**
- **No watermark**
- **No extra person** (only the original subject)
- **No cracks or scars across facial features** (glass damage would obscure identity)

**Canvas Preservation:**
- Preserve input image's native 4:5 canvas shape exactly
- Do not crop or extend
- Maintain original aspect ratio
- Perfect fit to provided dimensions

---

## Style Characteristics

### Stained Glass Elements

**Traditional Craftsmanship:**
- Handcrafted appearance
- Individual glass pieces
- Visible lead came joints
- Authentic stained glass technique

**Color Palette:**
- Rich jewel tones
- Deep saturated colors
- Translucent quality
- Warm and cool balance

**Light Transmission:**
- Backlit effect
- Glowing luminosity
- Light passing through glass
- Radiant quality

**Texture:**
- Smooth glass surfaces
- Subtle variations in color
- Light reflections
- Depth and dimension

---

### Art Nouveau Influence

**Ornamental Frame:**
- Flowing organic lines
- Natural forms (leaves, vines, flowers)
- Elegant curves
- Decorative sophistication

**Symmetry:**
- Balanced composition
- Mirror-like arrangement
- Harmonious design
- Classical beauty

**Botanical Elements:**
- Floral motifs
- Natural patterns
- Gothic rose windows
- Medieval church art inspiration

---

### Gothic Architecture References

**Rose Window Background:**
- Circular geometric patterns
- Radiating designs
- Medieval cathedral windows
- Intricate tracery

**Religious Iconography:**
- Radiant halo
- Sacred art tradition
- Devotional portrait style
- Spiritual atmosphere

**Architectural Detail:**
- Gothic arches (implied in design)
- Structural beauty
- Mathematical precision
- Historical authenticity

---

## Application Tips

### Best Subject Types

**Ideal Subjects:**
- Clear facial features
- Good lighting in original
- Centered composition
- Neutral or simple backgrounds
- Professional headshots
- Portrait photography

**Works Well With:**
- Human subjects of any age
- Pet portraits (adapt prompt for animals)
- Historical figures
- Celebrity portraits
- Family heirloom transformations

---

### Color Selection Strategy

**Warm Palette (Amber/Ruby dominant):**
- Sunset glow
- Warm skin tones
- Inviting atmosphere
- Rich golden light

**Cool Palette (Cobalt/Turquoise dominant):**
- Serene atmosphere
- Cool lighting
- Peaceful mood
- Ocean or sky themes

**Balanced Palette (Mix of all jewel tones):**
- Dynamic and varied
- Traditional stained glass look
- Maximum visual interest
- Harmonious blend

**Consider Subject:**
- Hair color influences glass tone choices
- Eye color can be emphasized in glass
- Clothing can use complementary colors
- Background contrasts with subject

---

### Customization Options

**Halo Variations:**
- Simple circular halo (traditional)
- Elaborate radiant burst
- Geometric patterns
- Floral crown instead of halo

**Background Styles:**
- Single large rose window
- Multiple smaller Gothic windows
- Abstract botanical patterns
- Geometric Art Nouveau designs

**Frame Options:**
- Heavy ornate Art Nouveau frame
- Delicate minimal framing
- Integrated into background design
- Extended decorative borders

**Lead Came Weight:**
- Thick bold leading (more graphic)
- Fine delicate leading (more detailed)
- Variable width leading (dynamic)
- Minimal leading (more painterly)

---

## Technical Specifications

### Input Requirements

**Image Format:**
- 4:5 portrait aspect ratio (recommended)
- High resolution preferred
- Clear facial features
- Good lighting
- Centered subject

**Subject Positioning:**
- Chest-up framing
- Centered in frame
- Face clearly visible
- Neutral expression recommended (or desired expression)

---

### Output Expectations

**Canvas Shape:**
- Maintains 4:5 portrait orientation
- Edge-to-edge composition
- No cropping or extension
- Exact aspect ratio preservation

**Resolution:**
- High quality output
- Detailed glass work
- Sharp lead came lines
- Clear facial features

**Color Depth:**
- Rich jewel tones
- Deep saturation
- Luminous quality
- Realistic glass appearance

---

## Use Cases

### Personal Projects

**Portrait Transformation:**
- Turn personal photos into art
- Gift for loved ones
- Unique profile pictures
- Creative self-expression

**Family Heritage:**
- Transform old family photos
- Create heirloom art pieces
- Restore and reimagine historical portraits
- Preserve memories in artistic form

**Memorial Art:**
- Honor loved ones
- Create lasting tributes
- Spiritual remembrance
- Beautiful commemoration

---

### Professional Applications

**Editorial Photography:**
- Premium magazine features
- Artistic portfolio pieces
- Creative editorial spreads
- High-end publication content

**Branding & Marketing:**
- Unique brand identity portraits
- Luxury product associations
- Artistic campaign imagery
- Premium positioning

**Religious & Spiritual:**
- Church art commissions
- Devotional imagery
- Spiritual publication artwork
- Sacred space decoration

**Art & Design:**
- Gallery exhibitions
- Print sales
- Digital art collections
- Interior design elements

---

### Commercial Uses

**Print Products:**
- Art prints for sale
- Canvas reproductions
- Greeting cards
- Calendar artwork

**Digital Products:**
- Desktop wallpapers
- Phone backgrounds
- Social media profile images
- Website hero images

**Commissioned Work:**
- Custom portrait commissions
- Corporate art installations
- Public space artwork
- Private collection pieces

---

## Style Variations

### Medieval Sacred Art

**Emphasis:**
- Stronger religious iconography
- More elaborate halos
- Biblical color symbolism
- Church art authenticity

**Modifications:**
```
Add: "in the style of medieval sacred art with traditional religious iconography"
Emphasize: "gilded halo, prayer gesture, sacred atmosphere"
```

---

### Art Nouveau Elegance

**Emphasis:**
- Stronger Art Nouveau frame
- More flowing organic lines
- Elaborate botanical elements
- Belle Époque sophistication

**Modifications:**
```
Add: "elaborate Art Nouveau frame with flowing organic vines and flowers"
Emphasize: "Alphonse Mucha inspired decorative elements"
```

---

### Gothic Cathedral Windows

**Emphasis:**
- Architectural precision
- Geometric patterns
- Multiple rose window motifs
- Cathedral grandeur

**Modifications:**
```
Add: "Gothic cathedral rose window design with precise architectural tracery"
Emphasize: "intricate geometric patterns, pointed arch elements"
```

---

### Contemporary Minimalist

**Emphasis:**
- Simplified glass panels
- Less ornate framing
- Modern clean lines
- Refined elegance

**Modifications:**
```
Add: "contemporary minimalist stained glass with clean geometric panels"
Reduce: "simplified Art Nouveau frame, minimal ornamentation"
```

---

## Comparison with Other Glass Styles

### vs. Divided Glass Effect

**Stained Glass Portrait:**
- Complete artistic transformation
- Subject becomes made of glass
- Ornamental frames and halos
- Religious/sacred art inspiration
- Preserves identity while transforming medium

**Divided Glass:**
- Fractured/shattered appearance
- Subject behind or within broken glass
- Typically no frame elements
- Modern conceptual aesthetic
- Less about transformation, more about distortion/fragmentation

---

### vs. Double Exposure

**Stained Glass Portrait:**
- Single cohesive glass artwork
- Material transformation (person → glass)
- Structured lead came boundaries
- Traditional craft aesthetic

**Double Exposure:**
- Two images blended/overlaid
- Transparency and overlap
- Surreal dreamlike quality
- Photography-based technique

---

### vs. Thermal Imaging

**Stained Glass Portrait:**
- Artistic glass medium
- Warm transmitted light
- Jewel tone color palette
- Handcrafted appearance

**Thermal:**
- Heat signature visualization
- False color temperature mapping
- Scientific/technical aesthetic
- Digital sensor simulation

---

## Troubleshooting

### Problem: Face not recognizable after transformation

**Solution:**
- Emphasize in prompt: "preserve the supplied person's recognizable face"
- Add: "maintain clear facial features and identity"
- Use higher quality input image
- Ensure original photo has clear visible features

---

### Problem: Too abstract, loses portrait quality

**Solution:**
- Emphasize: "attractive anatomically correct face"
- Add: "photorealistic facial features rendered in glass"
- Specify: "natural eyes, clear expression"
- Balance artistic transformation with identity preservation

---

### Problem: Background doesn't look like rose window

**Solution:**
- Specify: "Gothic cathedral rose window background with circular geometric patterns"
- Add: "intricate botanical tracery in the style of Notre-Dame rose windows"
- Request: "radiating geometric design with floral motifs"

---

### Problem: Glass doesn't look luminous or backlit

**Solution:**
- Emphasize: "warm transmitted light glowing through glass"
- Add: "backlit stained glass with radiant luminosity"
- Specify: "light source behind glass creating glowing effect"
- Request: "translucent jewel-like glass with inner glow"

---

### Problem: Lead came looks too thin or absent

**Solution:**
- Emphasize: "elegant dark lead came defining all glass panels"
- Add: "thick black leading outlining each glass piece"
- Specify: "traditional stained glass construction with visible joints"

---

### Problem: Colors too muted or dull

**Solution:**
- Emphasize: "jewel-like" and list all color names
- Add: "deeply saturated vibrant glass colors"
- Specify: "rich cobalt blue, glowing ruby red, luminous emerald green"
- Request: "maximum color saturation and brilliance"

---

### Problem: Canvas aspect ratio changes

**Solution:**
- Explicitly state: "Preserve the input image's native 4:5 canvas shape exactly"
- Add: "do not crop, extend, or alter aspect ratio"
- Verify input image is actually 4:5 before processing

---

### Problem: Unwanted text or watermarks appear

**Solution:**
- Prompt already prohibits: "no text, logo, watermark"
- Add if needed: "absolutely no text elements of any kind"
- Specify: "pure visual artwork with no typography"

---

## Pro Tips

### For Best Results

1. **Start with quality input:**
   - Well-lit portrait
   - Clear facial features
   - Simple or neutral background
   - Centered composition
   - 4:5 aspect ratio if possible

2. **Emphasize identity preservation:**
   - Repeat subject description if needed
   - Mention specific features (glasses, beard, hairstyle)
   - Request recognizable likeness

3. **Control glass complexity:**
   - More glass panels = more detailed
   - Larger panels = cleaner, bolder look
   - Lead came thickness affects style

4. **Balance transformation and recognition:**
   - Too abstract = loss of identity
   - Too realistic = doesn't look like glass
   - Find sweet spot in prompt emphasis

5. **Consider color harmony:**
   - Warm tones for inviting feel
   - Cool tones for serene atmosphere
   - Balanced palette for traditional look

---

## Example Variations

### Classic Cathedral Portrait

```
Transform the supplied 4:5 portrait into a traditional Gothic cathedral stained-glass window portrait. Preserve the person's recognizable face, eyes, hairstyle, hair color, skin tone, age and expression. Replace entire canvas with luminous handcrafted stained glass: face, hair, clothing, gilded circular halo, Gothic rose-window background with Notre-Dame inspired tracery, ornate medieval frame, thick dark lead came, jewel-like cobalt, ruby, amber, emerald and violet glass, warm transmitted light from behind. Centered chest-up symmetrical composition filling canvas edge to edge. Attractive anatomically correct face; no text, logo, watermark, extra person, cracks or scars. Preserve 4:5 canvas exactly.
```

---

### Art Nouveau Masterpiece

```
Transform the supplied 4:5 portrait into a premium Art Nouveau stained-glass portrait in the style of Alphonse Mucha. Preserve the person's recognizable face, eyes, hairstyle, hair color, skin tone, age and expression. Replace entire canvas with luminous handcrafted stained glass: face, hair, clothing, radiant halo with flowing rays, botanical background with lilies and vines, elaborate Art Nouveau frame with organic curves and floral elements, elegant dark lead came, jewel-like turquoise, amber, emerald, ruby and violet glass, warm transmitted light. Centered chest-up symmetrical composition filling canvas edge to edge. Attractive anatomically correct face; no text, logo, watermark. Preserve 4:5 canvas exactly.
```

---

### Contemporary Minimalist Glass

```
Transform the supplied 4:5 portrait into a contemporary minimalist stained-glass portrait. Preserve the person's recognizable face, eyes, hairstyle, hair color, skin tone, age and expression. Replace entire canvas with luminous handcrafted stained glass: face, hair, clothing, simple circular halo, clean geometric background, minimal modern frame, elegant dark lead came with bold clean lines, jewel-like cobalt, turquoise, amber and emerald glass, cool transmitted light. Centered chest-up symmetrical composition filling canvas edge to edge. Attractive anatomically correct face; no text, logo, watermark. Preserve 4:5 canvas exactly.
```

---

## Inspiration References

**Historical Stained Glass:**
- Notre-Dame Cathedral rose windows (Paris)
- Chartres Cathedral windows (France)
- Sainte-Chapelle (Paris)
- Tiffany Studios stained glass (USA)

**Art Movements:**
- Art Nouveau (1890-1910)
- Gothic Revival (19th century)
- Arts and Crafts Movement
- Pre-Raphaelite Brotherhood

**Artists:**
- Alphonse Mucha (Art Nouveau)
- Louis Comfort Tiffany (glass artistry)
- William Morris (Arts and Crafts)
- Marc Chagall (modern stained glass)

---

## Quick Reference

**Core Style:** Premium editorial stained-glass identity portrait

**Key Elements:**
- Luminous handcrafted glass
- Dark lead came structure
- Jewel-tone color palette (cobalt, turquoise, amber, ruby, emerald, violet)
- Radiant halo
- Gothic rose-window background
- Art Nouveau ornamental frame
- Warm transmitted light

**Composition:** Centered chest-up symmetrical, edge-to-edge

**Canvas:** 4:5 portrait (preserved exactly)

**Identity:** Preserved (recognizable face, features, expression)

**Prohibitions:** No text, logo, watermark, extra people, facial damage

**Best For:** Portrait transformation, artistic gifts, editorial imagery, religious art, premium commissions
