# Interior Design Prompts - Complete Collection Index

A comprehensive collection of professional interior design prompts including 13 living room style prompts for architectural documentation and photorealistic rendering, plus 15 professional design workflow tools covering the complete process from client consultation through final presentation.

## Overview

This collection provides two types of prompts:

**Style Prompts (13):** Detailed specifications for creating professional architectural drawings and photorealistic renders of complete living room interiors in various styles. Each generates:

- Front, Back, Left, Right elevations
- Plan View (top-down)
- Sections A-A / B-B
- Accurate dimensions and annotations
- Material schedules
- Lighting schedules
- Furniture specifications
- CAD-style professional presentations

**Process & Tool Prompts (15):** Professional workflow tools for interior designers covering concept generation, space planning, material selection, colour palettes, client presentations, budget planning, and technical documentation.

---

## Style Categories

### 1. **Luxurious Classical Living Room**

[luxurious-classical-living-room.md](luxurious-classical-living-room.md)

**Style:** European palace-inspired, ornate detailing, rich materials  
**Color Palette:** Cream, gold, emerald green, ivory, warm neutrals  
**Key Features:** Crown molding, coffered ceilings, chandeliers, symmetry, marble floors  
**Best For:** Formal entertaining spaces, high-end residential, luxury hotels

---

### 2. **Modern Minimalist Living Room**

[modern-minimalist-living-room.md](modern-minimalist-living-room.md)

**Style:** Clean lines, minimal ornamentation, functional beauty  
**Color Palette:** White, light gray, black accents, natural wood tones  
**Key Features:** Open space, concealed storage, integrated lighting, seamless surfaces  
**Best For:** Contemporary homes, urban apartments, modern lofts

---

### 3. **Mid-Century Modern Living Room**

[mid-century-modern-living-room.md](mid-century-modern-living-room.md)

**Style:** 1950s-1960s revival, organic modernism, retro futurism  
**Color Palette:** Warm wood tones, burnt orange, mustard yellow, teal, olive green  
**Key Features:** Tapered legs, organic curves, geometric patterns, exposed beams  
**Best For:** Vintage enthusiasts, collectors, retro-modern homes

---

### 4. **Industrial Loft Living Space**

[industrial-loft-living-space.md](industrial-loft-living-space.md)

**Style:** Raw materials, exposed structure, urban edginess  
**Color Palette:** Gray, black, brown leather, aged metals, warm Edison bulbs  
**Key Features:** Exposed brick, steel beams, concrete floors, high ceilings  
**Best For:** Warehouse conversions, urban lofts, studio spaces

---

### 5. **Scandinavian Hygge Living Room**

[scandinavian-hygge-living-room.md](scandinavian-hygge-living-room.md)

**Style:** Light, airy, cozy, functional simplicity with warmth  
**Color Palette:** White, soft gray, natural wood, black accents, muted pastels  
**Key Features:** Natural light maximization, organic shapes, cozy textiles  
**Best For:** Small spaces, northern climates, minimalist homes

---

### 6. **Art Deco Glamour Living Room**

[art-deco-glamour-living-room.md](art-deco-glamour-living-room.md)

**Style:** 1920s-1930s glamour, geometric luxury, Hollywood Regency  
**Color Palette:** Black, gold, emerald green, deep blue, cream, burgundy  
**Key Features:** Stepped forms, sunburst motifs, chevron patterns, mirrored surfaces  
**Best For:** Cocktail lounges, entertainment spaces, glamorous residences

---

### 7. **Bohemian Eclectic Living Room**

[bohemian-eclectic-living-room.md](bohemian-eclectic-living-room.md)

**Style:** Layered textures, global influences, artistic freedom  
**Color Palette:** Rich jewel tones, warm earth tones, pops of vibrant color  
**Key Features:** Mix of patterns, vintage and handmade items, plants, textiles  
**Best For:** Creative spaces, artist studios, collected interiors

---

### 8. **Japandi Zen Living Room**

[japandi-zen-living-room.md](japandi-zen-living-room.md)

**Style:** Japanese + Scandinavian fusion, minimal warmth  
**Color Palette:** Warm neutrals, black accents, natural wood, soft earth tones  
**Key Features:** Low furniture, clean lines, negative space, craftsmanship  
**Best For:** Meditation spaces, calm retreats, mindful living

---

### 9. **Coastal Mediterranean Living Room**

[coastal-mediterranean-living-room.md](coastal-mediterranean-living-room.md)

**Style:** Breezy elegance, seaside relaxation, European coastal charm  
**Color Palette:** White, cream, soft blue, sandy beige, weathered wood  
**Key Features:** Natural textures, light fabrics, nautical accents, indoor-outdoor flow  
**Best For:** Beach houses, coastal homes, vacation properties

---

### 10. **Transitional Elegant Living Room**

[transitional-elegant-living-room.md](transitional-elegant-living-room.md)

**Style:** Timeless blend of traditional and contemporary  
**Color Palette:** Soft neutrals, warm grays, cream, taupe  
**Key Features:** Mix of curved and straight lines, classic with modern touches  
**Best For:** Family homes, versatile spaces, timeless appeal

---

### 11. **Modern Farmhouse Living Room**

[modern-farmhouse-living-room.md](modern-farmhouse-living-room.md)

**Style:** Rustic charm meets modern simplicity, agricultural heritage  
**Color Palette:** White, black, natural wood, warm neutrals  
**Key Features:** Shiplap walls, barn doors, exposed beams, modern fixtures  
**Best For:** Rural homes, suburban houses, open concept living

---

### 12. **Contemporary Luxury Living Room**

[contemporary-luxury-living-room.md](contemporary-luxury-living-room.md)

**Style:** Sophisticated simplicity, premium materials, architectural precision  
**Color Palette:** Monochromatic with rich textures, deep tones, metallic accents  
**Key Features:** Statement materials, integrated technology, custom details  
**Best For:** High-end residences, penthouses, luxury condominiums

---

### 13. **English Country Cottage Living Room**

[english-country-cottage-living-room.md](english-country-cottage-living-room.md)

**Style:** Cozy charm, layered patterns, collected elegance  
**Color Palette:** Soft florals, warm creams, sage green, rose pink, butter yellow  
**Key Features:** Floral fabrics, antique furniture, chintz, exposed beams, fireplaces  
**Best For:** Cottages, country homes, traditional residences

---

## Professional Design Tools & Process Prompts

In addition to the living room style prompts above, this collection includes professional tools for the interior design process, from initial client consultation through final presentation.

### 01. **Interior Design Concept Generator**

[01.concept-generator.md](01.concept-generator.md)

**Purpose:** Complete design concept creation for any room and style  
**Includes:** Colour palette, furniture direction, lighting, materials, décor, budget alternatives  
**Platform:** ChatGPT (GPT-4)  
**Best For:** Initial concepts, client presentations, exploring design directions

---

### 02. **Room Makeover Ideas**

[02.room-makeover-ideas.md](02.room-makeover-ideas.md)

**Purpose:** Analyze existing rooms and provide transformation recommendations  
**Includes:** Layout, wall colours, lighting upgrades, storage, styling changes  
**Platform:** ChatGPT (GPT-4o with vision)  
**Best For:** Before/after planning, budget refreshes, renovation projects

---

### 03. **Space Planning Expert**

[03.space-planning-expert.md](03.space-planning-expert.md)

**Purpose:** Create efficient furniture layouts based on room dimensions  
**Includes:** Circulation, zoning, seating, storage, natural light optimization  
**Platform:** ChatGPT (GPT-4)  
**Best For:** New homes, furniture arrangement, spatial challenges

---

### 04. **Material & Finish Selector**

[04.material-finish-selector.md](04.material-finish-selector.md)

**Purpose:** Recommend materials and finishes for any interior style  
**Includes:** Flooring, walls, countertops, cabinetry, fabrics, metals, durability notes  
**Platform:** ChatGPT (GPT-4)  
**Best For:** Renovation planning, material selection, finish schedules

---

### 05. **Colour Palette Creator**

[05.colour-palette-creator.md](05.colour-palette-creator.md)

**Purpose:** Generate 5 professional colour schemes with hex codes  
**Includes:** Wall colours, furniture tones, accents, décor, hex codes  
**Platform:** ChatGPT (GPT-4)  
**Best For:** Paint selection, colour coordination, mood boards

---

### 06. **Mood Board Generator**

[06.mood-board-generator.md](06.mood-board-generator.md)

**Purpose:** Curate furniture, fabrics, textures, lighting, art, and plants  
**Includes:** Complete mood board elements, style story, cohesive explanation  
**Platform:** ChatGPT (GPT-4)  
**Best For:** Client presentations, Pinterest boards, design direction

---

### 07. **Client Presentation Writer**

[07.client-presentation-writer.md](07.client-presentation-writer.md)

**Purpose:** Write premium client presentations for design projects  
**Includes:** Concept story, inspiration, layout, materials, mood board, narrative  
**Platform:** ChatGPT (GPT-4)  
**Best For:** Winning clients, design proposals, professional presentations

---

### 08. **Budget Interior Planner**

[08.budget-interior-planner.md](08.budget-interior-planner.md)

**Purpose:** Design beautiful rooms within specific budgets  
**Includes:** Cost breakdown, priorities, alternatives, savings tips, itemized lists  
**Platform:** ChatGPT (GPT-4)  
**Best For:** Budget projects, cost estimation, DIY planning

---

### 09. **Lighting Design Assistant**

[09.lighting-design-assistant.md](09.lighting-design-assistant.md)

**Purpose:** Create professional layered lighting plans  
**Includes:** Ambient, task, accent, decorative lighting, Kelvin, lumens, placement  
**Platform:** ChatGPT (GPT-4)  
**Best For:** Lighting upgrades, new construction, renovation planning

---

### 10. **3D Render Improvement Prompt**

[10.3d-render-improvement.md](10.3d-render-improvement.md)

**Purpose:** Analyze and improve interior renders  
**Includes:** Lighting, materials, realism, composition, textures, reflections, camera  
**Platform:** ChatGPT (GPT-4o with vision)  
**Best For:** Refining AI renders, portfolio quality, pre-client polish

---

### 11. **Interior Design Content Creator**

[11.content-creator.md](11.content-creator.md)

**Purpose:** Generate 30 Instagram content ideas for design businesses  
**Includes:** Reels, carousels, hooks, captions, CTAs, SEO angles  
**Platform:** ChatGPT (GPT-4)  
**Best For:** Social media planning, content calendars, marketing

---

### 12. **Client Consultation Assistant**

[12.client-consultation-assistant.md](12.client-consultation-assistant.md)

**Purpose:** Generate comprehensive client questionnaires  
**Includes:** Lifestyle, budget, preferences, family needs, storage, timeline, expectations  
**Platform:** ChatGPT (GPT-4)  
**Best For:** Client intake, project discovery, consultation preparation

---

### 13. **Luxury Presentation Board**

[13.luxury-presentation-board.md](13.luxury-presentation-board.md)

**Purpose:** Transform renders into luxury editorial presentation boards  
**Includes:** Material palette, specs, elevations, floor plans, Swiss grid layout  
**Platform:** Midjourney, DALL-E 3  
**Best For:** Client deliverables, portfolio, competitions, Behance

---

### 14. **Architectural Presentation Board**

[14.architectural-presentation-board.md](14.architectural-presentation-board.md)

**Purpose:** Create premium technical documentation boards  
**Includes:** Furniture specs, material palette, floor plan, millwork diagrams, elevations  
**Platform:** Midjourney, DALL-E 3  
**Best For:** Technical presentations, RFP submissions, construction docs

---

### 15. **Bathroom Presentation Board**

[15.bathroom-presentation-board.md](15.bathroom-presentation-board.md)

**Purpose:** Transform bathroom renders into architectural presentation boards  
**Includes:** Fixture schedule, material palette, floor plan, elevations, sections, details  
**Platform:** Midjourney, DALL-E 3  
**Best For:** Bathroom projects, hospitality design, renovation presentations

---

### 16. **Kitchen Isometric Documentation Board**

[BOARDS.PRESENTATION.kitchen-isometric-documentation-board.md](BOARDS.SINGLE-ANGLE/BOARDS.PRESENTATION.kitchen-isometric-documentation-board.md)

**Purpose:** Transform a kitchen image or floor plan into a vertical architectural presentation board

**Includes:** Isometric view, floor plan, elevations, detail views, furniture schedule, lighting plan, material palette

**Platform:** Image generation models with reference-image support

**Best For:** Kitchen portfolios, client presentations, design reviews, social media boards

---

### 17. **3x3 Interior Camera-View Contact Sheet**

[BOARDS.CONTACT-SHEET.3x3-interior-camera-views.md](BOARDS.MULTI-ANGLE/BOARDS.CONTACT-SHEET.3x3-interior-camera-views.md)

**Purpose:** Generate nine camera views of one unchanged interior in a single contact sheet

**Includes:** Zoom, wide, right, left, back, front, artwork, lighting, and table views

**Platform:** Image generation models with reference-image and multi-panel support

**Best For:** Interior coverage studies, architectural presentations, portfolio boards, visual continuity

---

## How to Use These Prompts

### For Architectural Drawings:

Each prompt includes a detailed formula for generating professional architectural documentation. Use the provided specifications to create:

1. **Elevation Views** - Front, Back, Left, Right with dimensions
2. **Plan View** - Top-down layout with furniture placement
3. **Sections** - A-A and B-B cuts showing vertical details
4. **Schedules** - Materials, finishes, lighting, furniture
5. **Annotations** - CAD-style professional notes

### For Photorealistic Renders:

Each prompt also includes rendering parameters for creating ultra-realistic 8K visualizations with appropriate:

- Lighting conditions
- Material textures
- Camera settings
- Atmospheric qualities
- Photography style

### Customization:

All prompts are structured to allow easy customization:

- Adjust room dimensions
- Modify color palettes
- Change specific furniture pieces
- Alter material specifications
- Add or remove architectural features

---

## Technical Specifications

**Drawing Standards:**

- Scale: 1/4" = 1'-0" for plans and elevations
- Scale: 1/2" = 1'-0" for detailed sections
- All dimensions in feet and inches
- CAD-style professional presentation
- Material callouts and specifications

**Rendering Standards:**

- Resolution: 8K minimum
- Professional interior photography aesthetic
- Appropriate lighting for each style
- Material texture emphasis
- Architectural photography composition

---

## Style Selection Guide

**Choose Based on:**

### Budget Level:

- **Budget-Friendly:** Scandinavian, Modern Minimalist, Modern Farmhouse
- **Mid-Range:** Transitional, Coastal, Bohemian
- **High-End:** Contemporary Luxury, Classical, Art Deco

### Lifestyle:

- **Family Living:** Modern Farmhouse, Transitional, Coastal
- **Entertaining:** Classical, Art Deco, Transitional
- **Relaxation:** Scandinavian, Japandi, English Country
- **Creative/Artistic:** Bohemian, Mid-Century Modern, Industrial

### Space Size:

- **Small Spaces:** Scandinavian, Modern Minimalist, Japandi
- **Medium Spaces:** Most styles adapt well
- **Large Spaces:** Classical, Contemporary Luxury, Industrial Loft

### Maintenance Level:

- **Low Maintenance:** Modern Minimalist, Contemporary, Scandinavian
- **Moderate:** Most transitional and modern styles
- **High Maintenance:** English Country, Classical, Bohemian

---

## Quick Reference Matrix

| Style               | Complexity  | Pattern Use | Color Intensity | Formality     |
| ------------------- | ----------- | ----------- | --------------- | ------------- |
| Classical           | High        | Low-Medium  | Medium          | Formal        |
| Modern Minimalist   | Low         | None-Low    | Low             | Casual-Formal |
| Mid-Century Modern  | Medium      | Medium      | High            | Casual        |
| Industrial          | Medium      | Low         | Low             | Casual        |
| Scandinavian        | Low         | Low         | Low             | Casual        |
| Art Deco            | High        | High        | High            | Formal        |
| Bohemian            | High        | Very High   | Very High       | Casual        |
| Japandi             | Low         | None-Low    | Low             | Casual        |
| Coastal             | Low-Medium  | Low-Medium  | Low-Medium      | Casual        |
| Transitional        | Medium      | Low-Medium  | Low-Medium      | Semi-Formal   |
| Modern Farmhouse    | Medium      | Low-Medium  | Low-Medium      | Casual        |
| Contemporary Luxury | Medium-High | None-Low    | Low-Medium      | Formal        |
| English Country     | High        | Very High   | Medium-High     | Semi-Formal   |

---

## Version Information

**Collection:** Interior Design Prompts  
**Total Style Prompts:** 13 (Living room designs)  
**Total Process/Tool Prompts:** 15 (Professional design workflow)  
**Total Prompts:** 28  
**Format:** Markdown (.md)  
**Use Case:** AI image generation, architectural documentation, design process, client management  
**Updated:** 2026

---

## Notes

- **Style Prompts (1-13):** Architectural documentation and photorealistic rendering for living room designs
- **Process/Tool Prompts (01-15):** Professional workflow tools from client consultation through final presentation
- All prompts preserve design authenticity while allowing customization
- Suitable for use with ChatGPT, Midjourney, DALL-E, and other professional AI tools
- Each prompt includes comprehensive specifications and best practices
- Designed for both architectural documentation and complete design business workflow
- Based on professional interior design principles and standards

---

_For questions about specific styles or customization options, refer to the individual prompt files for detailed specifications._
